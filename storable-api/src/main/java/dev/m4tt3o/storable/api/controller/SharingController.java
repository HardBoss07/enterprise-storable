...
public class SharingController {
    ...
    public ResponseEntity<List<AccessPrivilegeDto>> getPrivileges(
        ...
        log.info(
            "API request: get privileges for node: {} by user: {}",
            nodeId,
            userId
        );
        List<AccessPrivilege> privileges = sharingService.getPrivileges(
            nodeId,
            userId
        );
        return ResponseEntity.ok(
            fileApiMapper.toAccessPrivilegeDtoList(privileges)
        );
    }
    ...
    public ResponseEntity<AccessPrivilegeDto> shareNode(
        ...
        log.info(
            "API request: share node: {} with user: {} at level: {}",
            nodeId,
            request.targetUserId(),
            request.level()
        );
        AccessPrivilege privilege = sharingService.shareNode(
            nodeId,
            request.targetUserId(),
            request.level(),
            userId
    ...
    public ResponseEntity<AccessPrivilegeDto> updatePrivilege(
        ...
        log.info(
            "API request: update privilege for node: {} and user: {} to level: {}",
            nodeId,
            targetUserId,
            request.level()
        );
        AccessPrivilege privilege = sharingService.updatePrivilege(
            nodeId,
            targetUserId,
            request.level(),
            userId
    ...
    public ResponseEntity<Void> removePrivilege(
        ...
        log.info(
            "API request: remove privilege for node: {} and user: {}",
            nodeId,
            targetUserId
        );
