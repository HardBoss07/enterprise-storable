...
public class FileNodePersistenceImpl implements FileNodePersistence {
    ...
    public List<FileMetadataDto> findChildren(Long parentId, String ownerId) {
        log.debug(
            "Finding children for parent ID: {} and owner: {}",
            parentId,
            ownerId
        );
        ...
        Long targetParentId = (parentId == null || parentId == 0)
            ? 1L
            : parentId;
        List<FileNode> nodes = repository.findByParentIdAndAuthorizedOwner(
            targetParentId,
            ownerId
        );
    ...
    public List<FileMetadataDto> findChildrenGlobal(Long parentId) {
        ...
        Long targetParentId = (parentId == null || parentId == 0)
            ? 1L
            : parentId;
        List<FileNode> nodes = repository.findByParentIdAndIsDeletedFalse(
            targetParentId
        );
    ...
    @Transactional
    /** Saves a folder node. */
    public FileMetadataDto saveFolder(
        String name,
        Long parentId,
        String ownerId,
        String storageKey
    ...
    @Transactional
    /** Saves a file node. */
    public FileMetadataDto saveFile(
        String name,
        Long parentId,
        String ownerId,
        String storageKey,
    ...
    public Optional<FileMetadataDto> findFolder(
        String ownerId,
        Long parentId,
        String name
    ) {
    ...
    public Optional<FileMetadataDto> findByNameParentAndOwner(
        String name,
        Long parentId,
        String ownerId
    ) {
        log.debug(
            "Finding node: {} by parent: {} and owner: {}",
            name,
            parentId,
            ownerId
        );
    ...
    public void softDelete(Long id, String ownerId) {
        log.info(
            "Soft deleting node ID: {} and its children for owner: {}",
            id,
            ownerId
        );
        repository
    ...
    public void restore(Long id, String ownerId) {
        log.info(
            "Restoring node ID: {} and its children for owner: {}",
            id,
            ownerId
        );
        repository
    ...
    public void emptyTrash(String ownerId) {
        ...
        List<FileNode> trash = repository.findByOwnerIdAndIsDeletedTrue(
            ownerId
        );
    ...
    public FileMetadataDto rename(Long id, String newName, String ownerId) {
        log.info(
            "Renaming node ID: {} to {} for owner: {}",
            id,
            newName,
            ownerId
        );
        FileNode node = repository
            .findByIdAndAuthorizedOwner(id, ownerId)
            .orElseThrow(() ->
    ...
    public FileMetadataDto move(Long id, Long targetParentId, String ownerId) {
        log.info(
            "Moving node ID: {} to parent ID: {} for owner: {}",
            id,
            targetParentId,
            ownerId
        );
        ...
        if (
            ...
            repository
                .findByIdAndAuthorizedOwner(targetParentId, ownerId)
                .orElseThrow(() ->
                    new RuntimeException(
                        "Target parent not found or access denied: " +
                            targetParentId
                    )
                );
        }
        ...
        node.setParentId(
            (targetParentId == null || targetParentId == 0)
                ? null
                : targetParentId
        );
        return toDto(repository.save(node));
    ...
    public List<FileMetadataDto> searchGlobal(String query, String kind) {
        log.debug(
            "Searching globally for nodes with query: {} and kind: {}",
            query,
            kind
        );
        FileNode.NodeKind nodeKind = null;
    ...
    public boolean existsByNameAndParentGlobal(String name, Long parentId) {
        log.debug(
            "Checking global existence for name: {} and parent: {}",
            name,
            parentId
        );
        return repository
    ...
    public FileMetadataDto toggleFavorite(
        ...
        log.info(
            "Setting favorite status to {} for node ID: {} and owner: {}",
            isFavorite,
            id,
            ownerId
        );
